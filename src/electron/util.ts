...
export function mapSample(row: SqlValue[]): PartialSample {
  ...
  return {
    ...
    sample_code: row[5] ? String(row[5]) : undefined,
    colloquial_specie: String(row[6]),
    cultivar: String(row[7]),
    harvest_year: String(row[8]),
    mark: row[9] ? String(row[9]) : undefined,
    lot_number: row[10] ? String(row[10]) : undefined,
    lot_weight: row[11] ? String(row[11]) : undefined,
