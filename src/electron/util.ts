...
export function mapSample(row: SqlValue[]): FullSample {
  return {
    ...
    lot_weight: String(row[10]),
    test_end_date: String(row[11]),
    observations: row[12] ? String(row[12]) : undefined,
    sampling_date: row[13] ? String(row[13]) : undefined,
    other_references: row[14] ? String(row[14]) : undefined,
    seal_number: row[15] ? String(row[15]) : undefined,
    specie: row[16] ? String(row[16]) : undefined,
    other_deter: row[17] ? String(row[17]) : undefined,
